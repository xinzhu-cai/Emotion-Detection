...
function [f,lab,ev2p] = loadcnt_reSampling(rsRate,filename,varargin)
% rsRate is the reSampling rate.
if ~isempty(varargin)
	 r=struct(varargin{:});
else r = []; 
end;
...
rs_ldnsamples = floor(r.ldnsamples/rsRate);
% channel offset
% --------------
...
if type == 'cnt' 
      ...
      if h.channeloffset <= 1
          dat = zeros(h.nchannels,rs_ldnsamples);
          for i = 1:rs_ldnsamples
              dat(:,i)=fread(fid, h.nchannels, r.dataformat);
              fseek(fid, 4*(rsRate-1)*h.nchannels, 'cof');
          end
%  	  else
